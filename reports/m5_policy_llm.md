...
## 통합 대시보드 — 완료 (2026-09-24)

`src/viz/dashboard.py`가 히트맵·랭킹·정책 카드를 `outputs/dashboard.html` 하나로 묶는다 — 셋이 따로 놀던 문제(위 "남은 일" 2번이었던 것)를 해소했다. dataviz 스킬 팔레트(sequential blue, status 팔레트)를 그대로 쓰고, cluster_id(1~4)를 상태 뱃지(최우선/주의/보통/양호)로 매핑했다. 지도는 기존 `gap_heatmap.png`를 base64로 임베드(폴리곤 렌더링 재구현 없음), 랭킹·카드는 벡터 SVG/HTML로 직접 그려 검색으로 필터링 가능하게 했다(행정동 이름 검색 시 차트·표·카드가 동시에 좁혀짐). `scripts/run_pipeline.py --stage viz`에 연결.

## 남은 일

1. 그래도 사람이 29건 정책 카드를 한 번은 읽어보는 게 좋다 — 가드레일이 못 잡는 영역(근거 수치가 배경 근거와 미묘하게 다르게 인용됐는지)은 기계적으로 검증 안 됨.
2. `README.md` §1 "NLP" 단계 설명 문구를 이 구조(nlp 없이 policy가 동작)에 맞게 GM 담당자와 상의 후 수정.

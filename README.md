...
| 마일스톤 | 상태 | 산출물 |
|---|---|---|
| M0. 데이터 해상도 검증 게이트 | 진행 중 | `reports/m0_data_audit.md` |
| M1. 수집·정제 파이프라인 | 대기 | `data/processed/*.parquet` |
| M2. 2SFCA 접근성 프로토타입 | 대기 | 의료·금융 2종 접근성 지수 |
| M3. 격차 점수 + 가중치 반영 | 대기 | 행정동별 Gap Score |
| M4. NLP 수요 신호 추출 | 대기 | 결핍 유형 라벨 |
...
| 데이터 | 출처 | 용도 | 확보 |
|---|---|---|---|
| 지자체 외국인주민현황 | 행안부 / KOSIS | 외국인 수요 밀도 | ⬜ |
| 주민등록인구 (행정동) | 행안부 / KOSIS | 격차 점수 분모 | ⬜ |
| 행정동 경계 폴리곤 | 통계청 SGIS | 공간 조인 기준 | ⬜ |
| 전국다문화가족실태조사 + 코드북 | MDIS / data.go.kr | 수요 신호 추출 | ⬜ |
| 의료기관 현황 (좌표) | 심평원 / data.go.kr | 의료 인프라 공급 | ⬜ |
| 상가(상권)정보 | 소상공인시장진흥공단 | 금융·생활시설 좌표 | ⬜ |
